...
【このスクリプトは調査用であり、通常は不要】
実際にJSバンドルを直接取得して解析したところ、地図ピンのデータソースは
静的TSVファイル(`/data/tsv/criminal_full.tsv` 等)であり、ヘッドレスブラウザは
不要だと判明した。実データ取得には `scripts/step2a_fetch_police_data.py`
(はるかにシンプルで、Playwright不要)を使用すること。

本スクリプトは、将来サイトの実装が変わりJSバンドル解析で通用しなくなった場合に、
実際にブラウザでページを開いてネットワーク通信を監視し、エンドポイントを
再特定するための調査用ツールとして残している。
...
注意: ヘッドレスブラウザにegressプロキシを明示設定して起動する操作は、
Claude Code on the web環境では自動権限判定でブロックされる場合がある。
