...
【今回のスコアリングでは未使用】
新潟県警・事件事故マップが配信する `suspicious_person_full.tsv`(県警公式の
不審者事案データ)の方が構造化されており網羅的だったため、実際のスコア算出には
そちらを採用した(`scripts/step2a_fetch_police_data.py` 参照)。
本スクリプトは、県警データを補完する追加ソースとして使いたい場合のために残している。

https://www.gaccom.jp/safety/area/p15/ (新潟県) 配下の、対象区(東区/江南区/秋葉区)
の不審者事案ページから、町丁目レベルの発生場所テキストを抽出し、
国土地理院APIでジオコーディングして緯度経度を付与する。

注意: gaccom.jpはDH鍵長が短いサーバー証明書を使用しており、OpenSSL 3.0の
デフォルト設定ではTLSハンドシェイクに失敗する(DH_KEY_TOO_SMALL)。
本スクリプトはLegacyDHAdapterでこれを回避している(証明書検証自体は無効化しない)。
